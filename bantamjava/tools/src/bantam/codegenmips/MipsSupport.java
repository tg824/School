...
public class MipsSupport {
    ...
    /** Generate an ASCII string length 
      * (accounts for '\' which appear like separate characters)
      * @param ascii ASCII string
      * @return length of the assembly ASCII string
      * */
    public int getAsciiLength(String ascii) {
	int length = 0;
	for (int i = 0; i < ascii.length(); i++) {
	    if (ascii.charAt(i) == '\\') i++;
	    length++;
    ...
    /** Generate a generic binary operation
      ...
      * */
    private void genBinaryOp(String op, String destReg, String srcReg,
			     String operand2) {
	checkReg(destReg);
	checkReg(srcReg);
