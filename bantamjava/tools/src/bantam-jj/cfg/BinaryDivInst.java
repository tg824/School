...
/** A class representing a three-address code binary integer division instruction 
  ...
  * */
public class BinaryDivInst extends BinaryInst {
    /** BinaryDivInst constructor
      ...
      * */
    public BinaryDivInst(String destination, String leftSource, String rightSource) {
	super(TACInst.BINDIV, destination, leftSource, rightSource);
	...
	if (!TACInst.checkVar(leftSource) && !TACInst.checkIntConst(leftSource))
	    throw new IllegalArgumentException("Bad left source operand '" +
					       leftSource + "' in BinaryDivInst constructor; " +
	...
	if (!TACInst.checkVar(rightSource) && !TACInst.checkIntConst(rightSource))
	    throw new IllegalArgumentException("Bad right source operand '" +
					       rightSource + "' in BinaryDivInst constructor; " +
					       "must be a legal variable or int constant");
    }
	...
	if (!TACInst.checkVar(leftSource) && !TACInst.checkIntConst(leftSource))
	    throw new IllegalArgumentException("Bad left source operand '" +
					       leftSource + "' in BinaryDivInst.setSource; " +
					       "must be a legal variable or int constant");
	...
	if (!TACInst.checkVar(rightSource) && !TACInst.checkIntConst(rightSource))
	    throw new IllegalArgumentException("Bad right source operand '" +
					       rightSource + "' in BinaryDivInst.setSource; " +
					       "must be a legal variable or int constant");
