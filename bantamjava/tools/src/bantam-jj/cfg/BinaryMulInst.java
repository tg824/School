...
/** A class representing a three-address code binary integer multiplication instruction 
  * (BinaryMulInst).  A binary multiplication instruction consists of a destination variable
  ...
  * */
public class BinaryMulInst extends BinaryInst {
    /** BinaryMulInst constructor
      ...
      * */
    public BinaryMulInst(String destination, String leftSource, String rightSource) {
	super(TACInst.BINMUL, destination, leftSource, rightSource);
	...
	if (!TACInst.checkVar(leftSource) && !TACInst.checkIntConst(leftSource))
	    throw new IllegalArgumentException("Bad left source operand '" +
					       leftSource + "' in BinaryMulInst constructor; " +
	...
	if (!TACInst.checkVar(rightSource) && !TACInst.checkIntConst(rightSource))
	    throw new IllegalArgumentException("Bad right source operand '" +
					       rightSource + "' in BinaryMulInst constructor; " +
					       "must be a legal variable or int constant");
    }
	...
	if (!TACInst.checkVar(leftSource) && !TACInst.checkIntConst(leftSource))
	    throw new IllegalArgumentException("Bad left source operand '" +
					       leftSource + "' in BinaryMulInst.setSource; " +
					       "must be a legal variable or int constant");
	...
	if (!TACInst.checkVar(rightSource) && !TACInst.checkIntConst(rightSource))
	    throw new IllegalArgumentException("Bad right source operand '" +
					       rightSource + "' in BinaryMulInst.setSource; " +
					       "must be a legal variable or int constant");
