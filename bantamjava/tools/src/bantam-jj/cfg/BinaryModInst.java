...
/** A class representing a three-address code binary integer modulus instruction 
  * (BinaryModInst).  A binary modulus instruction consists of a destination variable
...
public class BinaryModInst extends BinaryInst {
    /** BinaryModInst constructor
      ...
      * */
    public BinaryModInst(String destination, String leftSource, String rightSource) {
	super(TACInst.BINMOD, destination, leftSource, rightSource);
	...
	if (!TACInst.checkVar(leftSource) && !TACInst.checkIntConst(leftSource))
	    throw new IllegalArgumentException("Bad left source operand '" +
					       leftSource + "' in BinaryModInst constructor; " +
	...
	if (!TACInst.checkVar(rightSource) && !TACInst.checkIntConst(rightSource))
	    throw new IllegalArgumentException("Bad right source operand '" +
					       rightSource + "' in BinaryModInst constructor; " +
					       "must be a legal variable or int constant");
    }
	...
	if (!TACInst.checkVar(leftSource) && !TACInst.checkIntConst(leftSource))
	    throw new IllegalArgumentException("Bad left source operand '" +
					       leftSource + "' in BinaryModInst.setSource; " +
					       "must be a legal variable or int constant");
	...
	if (!TACInst.checkVar(rightSource) && !TACInst.checkIntConst(rightSource))
	    throw new IllegalArgumentException("Bad right source operand '" +
					       rightSource + "' in BinaryModInst.setSource; " +
					       "must be a legal variable or int constant");
