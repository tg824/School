...
public class Main {
	...
	System.err.println("Usage: bantamc [-h] [-o <output_file>] [-t <architecture>] [-gc] [-int] [-opt <num>]");
	System.err.println("               [-dl] [-dp] [-ds] [-di] [-do] [-dc] [-sl] [-sp] [-ss] [-so] <input_files>");
	...
	if (targetType == TARG_MIPS) return "mips";
	else if (targetType == TARG_X86) return "x86";
	else if (targetType == TARG_JVM) return "jvm";
	else throw new RuntimeException("Internal error: bad target type (" + 
	...
	for (int i = 0; i < args.length; i++) {
	    ...
	    // if -dl, -dp, -ds, -di, -do, or -dc then set corresponding boolean to enable debugging for that phase
	    ...
	    // if -sl, -sp, -ss, -so then set corresponding boolean to stop compilation after that phase
	    ...
	    // if -opt then turn on optimization
		...
		if (i == args.length - 1) {
		    ...
		    System.err.println("Usage error: must specify an optimization level with -opt");
		    showHelp();
		}
		i++;
		...
		i++;
		// check if architecture is "mips", if so, set targetType to mips (0)
		if (args[i].equals("mips"))
		    targetType = TARG_MIPS;
		// check if architecture is "x86", if so, set targetType to x86 (1)
		else if (args[i].equals("x86"))
		    targetType = TARG_X86;
		// check if architecture is "jvm", if so, set targetType to jvm (2)
		    ...
		    System.err.println("             must be either 'mips', 'x86', or 'jvm'");
		    showHelp();
		}
	    }
		...
		// check if outFile ends in .s
		if (outFile.length() < 3 || !outFile.substring(outFile.length()-2).equals(".s")) {
		    // if not, then print error message and call showHelp() (which eventually exits)
		    System.err.println("Usage error: bad output file name: " + args[i]);
	...
	// currently optimization not supported for JVM target
	// (will change soon)
	if (opt > 0 && targetType == TARG_JVM) {
	    System.out.println("Optimization currently not supported with JVM target");
	    System.exit(-1);
	}
	...
	try {
	    ...
	    lexer.scan();
	    if (stopAfterLexing) {
		// if stopAfterLexing==true, then scan with print set to true and exit
		lexer.printTokens();
		System.exit(0);
	    }
	    ...
	    // optimization (if specified via -opt flag)
	    if (opt > 0) {
