...
/** A class representing a three-address code binary boolean OR instruction 
  * (BinaryOrInst).  A binary OR instruction consists of a destination variable
  ...
  * */
public class BinaryOrInst extends BinaryInst {
    /** BinaryOrInst constructor
      ...
      * */
    public BinaryOrInst(String destination, String leftSource, String rightSource) {
	super(TACInst.BINOR, destination, leftSource, rightSource);
	...
	if (!TACInst.checkVar(leftSource) && !TACInst.checkBoolConst(leftSource))
	    throw new IllegalArgumentException("Bad left source operand '" +
					       leftSource + "' in BinaryOrInst constructor; " +
	...
	if (!TACInst.checkVar(rightSource) && !TACInst.checkBoolConst(rightSource))
	    throw new IllegalArgumentException("Bad right source operand '" +
					       rightSource + "' in BinaryOrInst constructor; " +
					       "must be a legal variable or boolean constant");
    }
