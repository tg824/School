...
public class Optimizer {
    ...
    /** Optimize the program
      ...
      *       (a) if opt level > 1 then perform high-level 
      *           transformations as building cfg
      *   2 - if opt level > 2 then perform low-level opt.
    ...
    /** Print out program (used by Main -- already implemented)
      * Note: this method relies on orderedClassList, which is
      * constructed but empty.  Will need to add class tree nodes
      * to this vector.
      * */
    public void print() {
	...
	for (int i = 0; i < orderedClassList.size(); i++) {
		...
		Iterator iter = ctn.getASTNode().getMemberList().getIterator();
		while (iter.hasNext()) {
